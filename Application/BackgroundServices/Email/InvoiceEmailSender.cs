...
public class InvoiceEmailSender(
    ...
    ICreditInvoiceEmailTemplate creditTemplate) : BackgroundService
{
    ...
    private async Task SendOne(CancellationToken cancellation)
    {
        ...
        var trigger = await database.DistributionTriggers
            .Where(x => 
                x.DistributionTriggerKind == DistributionTriggerKind.InvoiceEmail &&
        ...
        catch (Exception ex)
        {
            ...
            logItemList.AddLogItem(new LogItem(ex)
            {
                ...
                Source = nameof(InvoiceEmailSender),
            });
        }
